...
public class AppConfig {
    ...
    public AgroSyncConfig getAgroSyncConfig() {
        if (cachedAgroSync == null) {
            cachedAgroSync = new AgroSyncConfig(
                    getTimezone(ConfigKeys.Agro.Sync.TIMEZONE, ZoneId.of("Asia/Krasnoyarsk")),
                    getInstant(ConfigKeys.Agro.Sync.FROM_DATE, "2023-01-01T00:00:00.000Z"),
                    getInstant(ConfigKeys.Agro.Sync.TO_DATE, "2027-01-01T00:00:00.000Z"),
    ...
    public Instant getInstant(String key, String defaultValue) {
        String envValue = getString(key, defaultValue);
        try {
            return Instant.parse(envValue);
        } catch (Exception e) {
            log.error("Ошибка парсинга даты для ключа {}: '{}'. Используется дефолтное значение.", key, envValue);
